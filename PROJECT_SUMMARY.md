...
- Live verification also found older persistent rows displaying English/raw fallback labels. The frontend now uses explicit Ukrainian display maps for leaderboard species, bait, depth, cast spots, and waters; the final follow-up build output was JS `1,022.81 kB` (gzip `274.06 kB`) with the same existing Vite size warning.
